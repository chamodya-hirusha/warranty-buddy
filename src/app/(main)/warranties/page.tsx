...
function WarrantiesContent() {
  const { warrantyViews, deleteWarranty, brands } = useData();
  ...
  const bMap = useMemo(() => new Map(brands.map((b) => [b.id, b.name])), [brands]);
  ...
  const list = useMemo(() => {
    ...
    return warrantyViews.filter((w) => {
      ...
      if (!term) return true;
      const brandName = w.product?.brandId ? bMap.get(w.product.brandId) : undefined;
      return [
        w.product?.name, brandName, w.product?.serial,
        w.customer?.name, w.customer?.phone, w.notes,
      ].some((v) => (v ?? "").toLowerCase().includes(term));
    });
  }, [warrantyViews, filter, q, bMap]);
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {w.product?.brandId && bMap.has(w.product.brandId) && (
                    <span className="text-xs text-muted-foreground">· {bMap.get(w.product.brandId)}</span>
                  )}
                </div>
                <p className="text-sm text-muted-foreground truncate">
                  {w.customer ? (
                    <Link href={`/customers/${w.customer.id}`} className="hover:text-foreground hover:underline">
                      {w.customer.name}
                    </Link>
...
export default function WarrantiesPage() {
  return (
    <Suspense fallback={<div>Loading warranties...</div>}>
      <WarrantiesContent />
    </Suspense>
  );
}
